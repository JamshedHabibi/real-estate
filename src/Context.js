...
class Provider extends Component {
		...
		fetch(
			...
			.then((json) => {
				this.setState({
					apiIsLoaded: true,
					properties: json,
					listings: json.response.listings,
				});
			})
			.then(() => {
	...
	propertyFilter = (apiListings, tempArr) => {
		for (let i = 0; i < apiListings.length; i++) {
			if (
				apiListings[i].property_type === this.state.propertyType &&
				apiListings[i].price >= this.state.minPrice &&
				tempArr.indexOf(apiListings[i]) === -1 &&
				apiListings[i].price <= this.state.maxPrice &&
				apiListings[i].bedroom_number <= this.state.maxBed &&
				apiListings[i].bedroom_number >= this.state.minBed
			) {
				tempArr.push(apiListings[i]);
				this.setState({ filteredProperties: tempArr });
			} else if (
				this.state.propertyType === 'all' &&
				apiListings[i].price >= this.state.minPrice &&
				tempArr.indexOf(apiListings[i]) === -1 &&
				apiListings[i].price <= this.state.maxPrice &&
				apiListings[i].bedroom_number <= this.state.maxBed &&
	...
	searchBoxFilter = (apiListings, tempArr) => {
		...
		tempArr = propertyFiltered.filter(
			(prop) => prop.new_keywords.indexOf(searchCriteria) > -1
		);
		this.setState({ keywords: searchCriteria }, () => {
			for (let j = 0; j < searchCriteria.length; j++) {
				for (let i = 0; i < apiListings.length; i++) {
					if (
						apiListings[i].new_keywords.indexOf(searchCriteria[j]) > -1 &&
						apiListings[i].property_type === this.state.propertyType &&
						apiListings[i].price >= this.state.minPrice &&
						tempArr.indexOf(apiListings[i]) === -1 &&
						apiListings[i].price <= this.state.maxPrice &&
						...
						apiListings[i].new_keywords.indexOf(searchCriteria[i]) === -1
					) {
						this.setState({ filteredProperties: tempArr });
					}
				}
	...
	propertyTypeFilter = () => {
		...
		let property = document
			.getElementById('property-type-box')
			.value.toLowerCase();
		let propertyFiltered = [...this.state.filteredProperties];
		let tempArr = propertyFiltered.filter(
			(prop) => prop.property_type === property
		);
	...
	minBedFilter = () => {
		...
		let propertyFiltered = [...this.state.filteredProperties];
		let tempArr = propertyFiltered.filter(
			(prop) => prop.bedroom_number >= minBedNo
		);
	...
	maxBedFilter = () => {
		...
		let propertyFiltered = [...this.state.filteredProperties];
		let tempArr = propertyFiltered.filter(
			(prop) => prop.bedroom_number <= maxBedNo
		);
